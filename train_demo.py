...
from fewshot_re_kit.framework import FewShotREFramework, setup_logger
...
def main():
    ...
    if opt.pair:
        train_data_loader = get_loader_pair(opt.train, sentence_encoder,
                                            ...
                                            encoder_name=encoder_name, num_workers=0)
    ...
    if opt.adv:
        ...
        framework = FewShotREFramework(train_data_loader, val_data_loader, test_data_loader, experiment_name, adv_data_loader,
                                       adv=opt.adv, d=d)
    else:
        framework = FewShotREFramework(train_data_loader, val_data_loader, test_data_loader, experiment_name)
    ...
    if model_name == 'proto':
        ...
        logger.info("HINT: SNAIL works only in PyTorch 0.3.1")
        ...
        device = opt.device if opt.device > -1 and torch.cuda.is_available() else None
        model = Pair(sentence_encoder, hidden_size=opt.hidden_size, device=device)
    else:
        raise NotImplementedError

    if not os.path.exists('checkpoint'):
        os.mkdir('checkpoint')
    ckpt = 'checkpoint/{}.pth.tar'.format(experiment_name)
    if opt.save_ckpt:
        ckpt = opt.save_ckpt
    ...
    if not opt.only_test:
        ...
        framework.train(model, experiment_name, batch_size, trainN, N, K, Q,
    ...
    acc = framework.eval(model, batch_size, N, K, Q, opt.test_iter, device=opt.device, na_rate=opt.na_rate, ckpt=ckpt, pair=opt.pair)
